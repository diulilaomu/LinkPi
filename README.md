...
LinkPi（灵枢）是一款 Android 原生应用，内置完整的 AI Agent 编排引擎。用户只需描述需求，AI 将自动规划、生成、调试并交付可运行的 HTML5 应用。支持 **Workbench 并发任务执行**、FAST/FULL 双模式生成、多阶段工具调用循环、5 种意图分类、10 种动态工具组、长期记忆、动态 API 模块（HTTP/TCP/UDP）、完整设备原生桥接、会话离线存储、局域网 P2P 分享、**SSH 远程终端（AI 模式 + 手动模式）**、**SFTP 文件浏览器**和**凭据加密管理**。
...
| **57 个 Agent 工具** | 文件操作、网络请求、设备控制、记忆管理、代码搜索、模块调用、SSH 远程管理等 |
...
│   ├── ModuleStorage.kt                    # 动态模块系统（HTTP/TCP/UDP CRUD+调用）  473
...
- **工作区快照**：存在活跃工作区时注入文件列表 + 代码预览（每文件前 30 行）
...
sshExec / ssh_execute              → ssh_exec
sshDisconnect / ssh_close          → ssh_disconnect
```
