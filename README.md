...
| **SSH 远程终端** | AI 模式（AI 建议命令→用户确认执行）+ 手动模式（直接输入命令），右滑/左滑切换模式 |
...
│   └── CredentialStorage.kt                # 凭据加密存储（AES-256-GCM）            95
│
├── network/
│   ├── AiConfig.kt                         # 多模型配置（加密存储/预设/迁移）         191
│   ├── AiService.kt                        # LLM 调用（modelOverride 并发安全/重试退避）327
...
│   └── GenerationService.kt               # 前台 Service（通知栏进度/防杀保活）      279
│
├── skill/
...
│   ├── PromptAssembler.kt                  # 提示词三维矩阵组装（FAST/FULL + 注入）    157
│   ├── IntentClassifier.kt                 # AI + 本地关键词双重意图分类               97
...
│
└── ui/
    ├── navigation/NavGraph.kt              # 状态路由 + 会话历史侧滑面板 + SSH 入口   592
    ├── theme/
    ...
    ├── chat/
    │   ├── ChatViewModel.kt                # 状态管理 + 会话 CRUD + Agent 调度       442
    │   └── ChatScreen.kt                   # 对话 UI + Markdown 渲染               1403
    ├── miniapp/
    ...
    │   ├── WorkbenchListScreen.kt          # 任务列表（创建/重跑/删除）               346
    │   ├── WorkbenchDetailScreen.kt        # 任务详情（文件浏览器/步骤时间线）        1259
    │   └── AppCreationCard.kt              # 快速创建卡片组件                         102
    ├── ssh/
    │   ├── SshViewModel.kt                 # SSH 终端状态管理（AI/手动双模式）         385
    │   └── SshScreen.kt                    # SSH 终端 UI（命令卡片/流式输出/快捷键） 1446
    ├── sftp/
    │   ├── SftpViewModel.kt                # SFTP 文件管理状态（多选/批量/日志）       396
    │   ├── SftpScreen.kt                   # SFTP 文件浏览器 UI（面包屑/操作菜单）    710
    │   └── FileEditorScreen.kt             # 远程文件编辑器（查找/替换/保存回远程）    384
    ├── permission/
...
└── 手动模式: ManualTerminalContent → SshViewModel → SshManager（直接 exec）
```
