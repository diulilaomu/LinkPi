...
LinkPi（灵枢）是一款 Android 原生应用，内置完整的 AI Agent 编排引擎。用户只需描述需求，AI 将自动规划、生成、调试并交付可运行的 HTML5 应用。支持 **Workbench 并发任务执行**、FAST/FULL 双模式生成、多阶段工具调用循环、5 种意图分类、10 种动态工具组、长期记忆、动态 API 模块（HTTP/TCP/UDP）、完整设备原生桥接、会话离线存储和局域网 P2P 分享。

**45 个 Kotlin 源文件，约 12,700 行代码。**
...
│
├── network/
│   ├── AiConfig.kt                         # 多模型配置（加密存储/预设/迁移）         218
│   └── AiService.kt                        # LLM 调用（modelOverride 并发安全/重试退避）311
│
├── agent/
│   ├── AgentOrchestrator.kt                # 三阶段 Agent 编排引擎（FAST/FULL 双模式）830
│   ├── AppInfoAgent.kt                     # 独立元数据代理（名称/描述/架构文档）      120
│   ├── ToolExecutor.kt                     # 工具执行器（49 工具 + 18 别名）         980
...
│   └── FileBlockParser.kt                  # FAST 模式结构化文件输出解析              62
│
├── workspace/
│   └── WorkspaceManager.kt                 # 多文件工作区（CRUD/快照/搜索/校验/清单）  925
│
├── workbench/
│   ├── WorkbenchEngine.kt                  # 独立生成引擎（并发安全/AppInfoAgent 集成）400
│   └── WorkbenchTaskStorage.kt             # 任务持久化（QUEUED/PLANNING/.../COMPLETED） 95
│
├── skill/
│   ├── BuiltInSkills.kt                    # 内置 Skill + 9 个系统模板（含 FAST 工作流）466
│   ├── SkillStorage.kt                     # 自定义 Skill JSON 持久化                 85
│   ├── ToolGroup.kt                        # 枚举定义 + 工具可见性规则（MODIFY 只读规划）160
│   ├── PromptAssembler.kt                  # 提示词三维矩阵组装（FAST/FULL + 注入）    175
│   ├── IntentClassifier.kt                 # AI + 本地关键词双重意图分类               135
...
└── ui/
    ...
    │   └── MiniAppScreen.kt                # WebView 运行器 + nativeFetch 注入       488
    ├── workbench/
    │   ├── WorkbenchViewModel.kt           # 并发任务状态管理 + 生命周期               190
    │   ├── WorkbenchListScreen.kt          # 任务列表（创建/重跑/删除）               310
    │   ├── WorkbenchDetailScreen.kt        # 任务详情（文件浏览器/步骤时间线）         1180
    │   └── AppCreationCard.kt              # 快速创建卡片组件                          95
    ├── skill/
...
MIT License — © Wenhe X Rnzy 2026.3.10
